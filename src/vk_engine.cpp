...
void VulkanEngine::draw() {
    ...
    // Transition draw to general and draw background
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd) {
    ...
    //
    //    get_current_frame()._delQueue.push_function([=, this]() { destroy_buffer(gpuSceneDataBuffer); });
    //
...
AllocatedBuffer VulkanEngine::create_buffer(size_t allocSize, VkBufferUsageFlags usage, VmaMemoryUsage memoryUsage) {
    ...
    AllocatedBuffer newBuf;// NOLINT(*-pro-type-member-init)
    VK_CHECK(vmaCreateBuffer(_allocator, &bufferInfo, &vmaallocinfo, &newBuf.buffer, &newBuf.allocation, &newBuf.info));
    return newBuf;
}
...
void VulkanEngine::init_descriptors() {
    ...
    for (int i = 0; i < FRAME_OVERLAP; ++i) {
        ...
        _mainDelQueue.push_function([&, i]() { _frames[i]._frameDescriptors.destroy_pools(_device); });
    }
}
...
void VulkanEngine::init_mesh_pipeline() {
    ...
    if (!vkutil::load_shader_module("../shaders/tex_image.frag.spv", _device, &triangleFragShader)) {
        fmt::println("[Mesh] Error when building the triangle fragment shader "
                     "module");
    } else {
    ...
    pplBuilder._pipelineLayout = _meshPipelineLayout;
    ...
    _meshPipeline = pplBuilder.build_pipeline(_device);
...
void VulkanEngine::init_default_data() {
    ...
    _greyImage =
            create_image((void *) &grey, VkExtent3D{1, 1, 1}, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_USAGE_SAMPLED_BIT);
    ...
    for (int x = 0; x < 16; x++) {
        for (int y = 0; y < 16; y++) { pixels[y * 16 + x] = ((x % 2) ^ (y % 2)) ? magenta : black; }
    }
    ...
    GLTFMetallic_Roughness::MaterialResources matRes;
    ...
    _mainDelQueue.push_function([=, this]() { destroy_buffer(materialConstants); });

    matRes.dataBuffer       = materialConstants.buffer;
    matRes.dataBufferOffset = 0;

    defaultData =
            metalRoughMaterial.write_material(_device, MaterialPass::MainColor, matRes, globalDescriptorAllocator);
}
...
void GLTFMetallic_Roughness::build_pipelines(VulkanEngine *engine) {
    ...
    vkutil::PipelineBuilder pplBuilder;
    pplBuilder.set_shaders(meshVertexShader, meshFragShader);
    pplBuilder.set_input_topology(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST);
    pplBuilder.set_polygon_mode(VK_POLYGON_MODE_FILL);
    pplBuilder.set_cull_mode(VK_CULL_MODE_NONE, VK_FRONT_FACE_CLOCKWISE);
    pplBuilder.set_multisampling_none();
    ...
    pplBuilder.set_color_attachment_format(engine->_drawImage.imageFormat);
    pplBuilder.set_depth_format(engine->_depthImage.imageFormat);

    pplBuilder._pipelineLayout = newLayout;

    opaquePipeline.pipeline = pplBuilder.build_pipeline(engine->_device);

    pplBuilder.enable_blending_additive();
    pplBuilder.enable_depthtest(false, VK_COMPARE_OP_GREATER_OR_EQUAL);

    transparentPipeline.pipeline = pplBuilder.build_pipeline(engine->_device);
